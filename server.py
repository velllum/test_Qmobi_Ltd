...
class SimpleHandler(BaseHTTPRequestHandler):
    ...
    def _fill_base_dict(self):
        ...
        - Если статус подтвердился, то заполняем словарь наполняем базовый словарь данными Переопределяем свойство
          base_data - основной словарь self.base_data, полями из свойства статуса и данными из конвертера
        - Если же нет
          то тогда предаем только статус об ошибке сервера self.status
